...
-- Đảm bảo cột SoNgayCongChuan hỗ trợ số thực (FLOAT) (Audit Fix)
IF EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID('dbo.PhieuLuong')
      AND name = 'SoNgayCongChuan'
...
BEGIN
    IF EXISTS (
        ...
        WHERE name = 'CK_PhieuLuong_SoNgayCongChuan'
          AND parent_object_id = OBJECT_ID('dbo.PhieuLuong')
    )
    BEGIN
        ALTER TABLE dbo.PhieuLuong
        DROP CONSTRAINT CK_PhieuLuong_SoNgayCongChuan;
    END;

    ALTER TABLE dbo.PhieuLuong
    ALTER COLUMN SoNgayCongChuan FLOAT NOT NULL;

    ALTER TABLE dbo.PhieuLuong
    ADD CONSTRAINT CK_PhieuLuong_SoNgayCongChuan
    CHECK (SoNgayCongChuan >= 0);

    PRINT 'Đã chuyển đổi cột SoNgayCongChuan sang kiểu FLOAT';
END;
GO

-- =============================================
-- 7. STORED PROCEDURES (LOGIC NÂNG CAO)
-- =============================================
    ...
    BEGIN TRY
        ...
        USING (
            SELECT 
                ...
                -- Khấu trừ không lương = SoNgayNghiKhongLuong * (LuongCoBan / SoNgayCongChuan)
                (ISNULL(np_kl.TongNgay, 0) * (lsl.LuongCoBan / @SoNgayCongChuan)) AS CacKhoanKhauTruKhac,
