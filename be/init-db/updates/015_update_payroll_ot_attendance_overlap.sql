    ...
    BEGIN TRY
        ...
        USING (
            SELECT 
                ...
                -- Làm thêm (OT quy đổi theo hệ số)
                ISNULL(ot.SoGioOTQuyDoi, 0) AS SoGioLamThem,
                
                -- Dữ liệu lương gốc
                lsl.LuongCoBan,
                
                -- Phụ cấp thực tế (được tính phân bổ theo tỉ lệ ngày công đi làm + nghỉ hưởng lương)
                ISNULL(
                    (ISNULL(cc.TongSoGioLam, 0) / @SoGioNgayChuan + ISNULL(np_hl.TongNgay, 0)) 
                    * (lsl.PhuCap / NULLIF(ISNULL(emp_work.SoNgayCongChuanCaNhan, 0), 0)), 
                    0
                ) AS PhuCap,
            ...
            -- Tổng hợp Làm thêm (OT) - Đã đối chiếu chấm công thực tế và tính giao nhau (overlap)
            LEFT JOIN (
                SELECT 
                    ot.MaNhanVienId,
                    SUM(
                        CASE 
                            WHEN cc.GioVao IS NOT NULL AND cc.GioRa IS NOT NULL AND cc.TrangThai IN (N'CoMat', N'DiMuon')
                            THEN 
                                -- 1. Quy đổi giờ Đăng ký thành DATETIME đầy đủ
                                -- RegStart: NgayLamThem + GioBatDau
                                -- RegEnd: NgayLamThem + GioKetThuc (Nếu GioKetThuc < GioBatDau thì cộng thêm 1 ngày)
                                CASE 
                                    -- Đảm bảo khoảng thời gian hợp lệ và lấy hiệu số của phần giao nhau
                                    WHEN 
                                        -- OverlapEnd (MIN của RegEnd và cc.GioRa)
                                        (CASE 
                                            WHEN (CASE WHEN ot.GioKetThuc < ot.GioBatDau THEN DATEADD(DAY, 1, DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioKetThuc), CAST(ot.NgayLamThem AS DATETIME))) ELSE DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioKetThuc), CAST(ot.NgayLamThem AS DATETIME)) END) < cc.GioRa 
                                            THEN (CASE WHEN ot.GioKetThuc < ot.GioBatDau THEN DATEADD(DAY, 1, DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioKetThuc), CAST(ot.NgayLamThem AS DATETIME))) ELSE DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioKetThuc), CAST(ot.NgayLamThem AS DATETIME)) END)
                                            ELSE cc.GioRa 
                                         END)
                                        > 
                                        -- OverlapStart (MAX của RegStart và cc.GioVao)
                                        (CASE 
                                            WHEN DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioBatDau), CAST(ot.NgayLamThem AS DATETIME)) > cc.GioVao 
                                            THEN DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', ot.GioBatDau), CAST(ot.NgayLamThem AS DATETIME))
                                            ELSE cc.GioVao 
                                         END)
                                    THEN
                                        DATEDIFF(
                ...
                FROM dbo.DonLamThem ot
                INNER JOIN dbo.ChamCong cc 
                    ON ot.MaNhanVienId = cc.MaNhanVienId 
                    AND ot.NgayLamThem = cc.NgayLamViec
                WHERE ot.TrangThai = 'Approved'
                  AND ot.NgayLamThem BETWEEN @StartDate AND @EndDate
                GROUP BY ot.MaNhanVienId
        ...
        WHEN MATCHED AND ISNULL(target.TrangThai, '') <> 'Paid' THEN
            UPDATE SET 
                ...
                ThueTNCN = CASE 
                    ...
                    ELSE 0 END,
                LuongThucNhan = CASE
                    -- THÊM BIẾN CHẶN: Nếu không làm ngày nào và không nghỉ hưởng lương thì thực nhận tự động bằng 0
                    WHEN (source.SoNgayCongThucTe + source.SoNgayNghiHuongLuong) = 0 THEN 0
                    WHEN (
